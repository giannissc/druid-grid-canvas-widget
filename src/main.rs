...
use druid::{
    theme, AppLauncher, Color, Data, Lens, LocalizedString, Widget, WidgetExt, WidgetId, WindowDesc,
};

use druid::widget::{
    Button, ControllerHost, CrossAxisAlignment, Flex, Label, MainAxisAlignment, Switch,
};

use druid_color_thesaurus::*;

use druid_grid_graph_widget::grid_canvas::{GridCanvas, GridCanvasData};
use druid_grid_graph_widget::panning::{PanController, PanDataAccess};
use druid_grid_graph_widget::snapping::{GridSnapData, GridSnapDataAccess, GridSnapPainter};
use druid_grid_graph_widget::utils::cassetta::{CassettePlayer, TapeItem};
use druid_grid_graph_widget::zooming::{ZoomController, ZoomDataAccess};
use druid_grid_graph_widget::{GridIndex, GridItem};
//////////////////////////////////////////////////////////////////////////////////////
// Constants
//////////////////////////////////////////////////////////////////////////////////////
...
impl GridSnapDataAccess for AppData {
    ...
    fn move_to_grid_position(&self, desired_position: druid::Point) -> druid::Point {
        self.grid_data
            .snap_data
            .move_to_grid_position_2(desired_position)
    }
}
...
fn main() {
    ...
    pattern.push_back(TapeItem::Add(
    ...
    let mut map: HashMap<GridIndex, (GridNodeType<Net>, Option<GridNodeType<Net>>)> =
        HashMap::new();
    ...
    pattern.push_back(TapeItem::BatchAdd(map));

    pattern.push_back(TapeItem::Add(
        GridIndex { row: 4, col: 4 },
        GridNodeType::StartNode(0),
        None,
    ));
    pattern.push_back(TapeItem::Add(
        GridIndex { row: 12, col: 4 },
        GridNodeType::TargetNode(0),
        None,
    ));

    pattern.push_back(TapeItem::Add(
        GridIndex { row: 8, col: 1 },
        GridNodeType::StartNode(1),
        None,
    ));
    pattern.push_back(TapeItem::Add(
        GridIndex { row: 8, col: 18 },
        GridNodeType::TargetNode(1),
        None,
    ));
    pattern.push_back(TapeItem::Add(
        GridIndex { row: 3, col: 19 },
        GridNodeType::TargetNode(1),
        None,
    ));
    pattern.push_back(TapeItem::Add(
        GridIndex { row: 6, col: 35 },
        GridNodeType::TargetNode(1),
        None,
    ));
    ...
    Flex::column()
        ...
        .with_child(
            Flex::row()
                ...
                .with_child(
                    Button::new("Add perimeter")
                        .lens(AppData::grid_data)
                        .on_click(|ctx, data, _env| {
                            data.grid_data.add_node_perimeter(
                                GridIndex { row: 5, col: 5 },
                                5,
                                5,
                                GridNodeType::Boundary,
                            );
                        }),
                )
                .with_child(
                    Button::new("Add pattern 1")
                        .lens(AppData::grid_data)
                        .on_click(|ctx, data, _env| {
                            let mut pattern = Vector::new();
                            for col in 35..40 {
                                for row in 10..15 {
                                    pattern.push_back(TapeItem::Add(
                                        GridIndex { row, col },
                                        GridNodeType::UnexploredNode(5),
                                        None,
                                    ));
                                    pattern.push_back(TapeItem::Add(
                                        GridIndex { row, col },
                                        GridNodeType::ExploredNode(5),
                                        Some(GridNodeType::UnexploredNode(5)),
                                    ));
                                }
                            }
        ...
        .with_child(
            Flex::row()
                ...
                .with_child(
                    Switch::new()
                        .lens(GridSnapData::grid_visibility)
                        .lens(GridCanvasData::snap_data)
                        .lens(AppData::grid_data),
                )
                .main_axis_alignment(MainAxisAlignment::SpaceBetween)
                .cross_axis_alignment(CrossAxisAlignment::Start)
